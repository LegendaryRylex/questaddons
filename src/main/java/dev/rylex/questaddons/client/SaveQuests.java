...
public final class SaveQuests {
    ...
    public static boolean downloadQuestFiles() {
        ...
        try {
            ...
            file.getTranslationManager().saveToNBT(path.resolve("lang"), true);

            String shown = "."
                    + dir.getPath()
                            .replace(minecraft.gameDirectory.getCanonicalFile().getAbsolutePath(), "");
            player.displayClientMessage(
                    Component.translatable("ftbquests.gui.saved_as_file", shown)
                            .withStyle(Style.EMPTY.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, shown))),
                    false);
            return true;
        } catch (Exception e) {
