...
public final class SplitView {
     ...
     */
    public static void draw(QuestScreen screen, GuiGraphicsExtractor graphics) {
        SplitViewPane current = pane;
        ...
        if (orientation == SplitOrientation.SIDE_BY_SIDE) {
            IconHelper.renderIcon(color, graphics, position, 0, DIVIDER_WIDTH, windowHeight(screen));
        } else {
            IconHelper.renderIcon(color, graphics, 0, position, windowWidth(screen), DIVIDER_WIDTH);
        }
    }
    ...
    public static boolean charTyped(BaseScreen screen, CharacterEvent event) {
        SplitViewPane current = pane;
        return current != null && root == screen && paneFocused && current.charTyped(event);
    }
